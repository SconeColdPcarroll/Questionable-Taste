const { json, verifySignedToken } = require('./_lib');
...
module.exports = async function handler(req, res) {
  ...
  const [meRes, topRes] = await Promise.all([
    ...
    fetch('https://api.spotify.com/v1/me/top/artists?limit=5&time_range=medium_term', { headers })
  ]);

  const me = await meRes.json();
  const top = await topRes.json();

  if (!meRes.ok) return json(res, meRes.status, { error: me.error?.message || 'Spotify profile failed' });
  if (!topRes.ok) return json(res, topRes.status, { error: top.error?.message || 'Spotify top artists failed' });
